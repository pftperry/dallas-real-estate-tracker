...
Everything shown here has already cleared the hard eligibility gate: price inside the $750K–$1M band, 3+ beds, 2+ baths, and either resolved zoning to **Mockingbird or Lakewood Elementary** or an address inside one of two **Lake Highlands geographic zones** (the drawn L Streets outline, or Lake Highlands Estates). Nothing that fails one of those is down-ranked; it is excluded before scoring, so these plays are already zoning-clean.
...
- **Lake Highlands Estates** &mdash; RISD, so the elementary gate cannot pass; promoted to a qualifying zone in its own right on 2026-08-03. Until then only its eastern sliver counted, the accidental overlap with the L Streets outline, so none of its listings had ever qualified. It is now the strongest-fitting area in the screen for the character-on-a-real-lot brief: its 1963-65 stock runs 2,900-3,200 sqft on 8-10k lots at $275-347/sqft, and it holds the top-ranked listing. ~$1.03M median means anything eligible is at the bottom of the local range, so check you are not simply buying the cheapest house on a premium street. Look at Highedge and Milltrail first.
