...
- **Scoring weights** — edit `pipeline/score.py` `WEIGHTS` (must sum to 1.0). Current set ranks the *house*, since the gate already settled location: $/sqft vs peers 30%, lot 20%, DOM 15%, vintage 15%, school confidence 10%, price position 10%. `lakewood_orbit` was removed from the score in 2026-08 — it carried 30% and supplied 36% of all discriminating power, which double-charged a preference the gate had already applied, burying the drawn Lake Highlands zone by construction. It is still reported per listing as context. To bias back toward Lakewood-side proximity, key a weight on the qualifying *basis* rather than reviving the 20-area orbit scale.
